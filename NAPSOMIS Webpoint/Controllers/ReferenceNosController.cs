...
namespace NAPSOMIS_Webpoint.Controllers
{
    public class ReferenceNosController : Controller
    {
        ...
        public ActionResult ReferenceNos(ReferenceNo rn)
        {
            ...
            string thefilter = ddate.ToString("MM") + ddate.ToString("yy");

            int StartNo = GetLastGeneratedRefNo(thefilter);
 
            string range = StartNo.ToString() + "-" + (StartNo + Quantity -1);

            string GeneratedStart = "W" + ddate.ToString("MM") + ddate.ToString("yy") + CompleteStartNo(StartNo); ;
            string GeneratedEnd = "";
                ...
                if (Quantity > 0)
                {
                    ...
                    {
                        int newNo = StartNo + i;
                        string NewReferenceNo = "W" + ddate.ToString("MM") + ddate.ToString("yy") + CompleteStartNo(newNo);
                ...
                mysingledbreference.GeneratedBy = "Default";
